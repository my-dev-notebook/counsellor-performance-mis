...
/**
 * Centred card for the sign-in and password pages: a centred title, an
 * optional one-line intro, then the form. The brand sits in `AuthHeader` above, not repeated here.
 */
        ...
        <main data-component="AuthCard" className="bg-canvas flex flex-1 items-center justify-center px-4 py-12">
            <div className="card card-pad stack w-full max-w-sm gap-5 p-7">
                <div className="text-center">
                    ...
                    {sub && <p className="t-sm ink-2 mt-1">{sub}</p>}
                </div>
                {children}
            </div>
        </main>
