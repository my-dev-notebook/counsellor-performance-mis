...
export default function UploadPreviewPage() {
    ...
    return (
        <div data-component="UploadPreviewPage" className="flex min-h-full flex-1 flex-col bg-background">
            ...
            <main className="mx-auto w-full max-w-7xl flex-1 space-y-6 px-4 py-6 sm:px-6 lg:px-8">
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        Drop a raw monthly workbook to preview how it parses. This is a scratch view only — nothing here
                        is saved to the database. To record official figures, use{" "}
            ...
            {state.status === "ready" && (
                <Footer sourceFileName={state.workbook.sourceFileName} monthLabel={state.workbook.monthLabel} />
            )}
        </div>
    );
}
