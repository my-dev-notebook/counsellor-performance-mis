...
export function buildCounsellor(params: BuildRowParams): BuildRowOutcome {
  ...
  const nameResult =
    columns.name !== undefined ? toStringOrNull(cells[columns.name]) : { value: null };
  ...
  const agencyResult =
    columns.agency !== undefined ? toStringOrNull(cells[columns.agency]) : { value: null };
  const dojResult = columns.doj !== undefined ? toDate(cells[columns.doj]) : { value: null };
  const emailResult =
    columns.email !== undefined ? toStringOrNull(cells[columns.email]) : { value: null };
  const targetResult =
    columns.target !== undefined ? toNumber(cells[columns.target]) : { value: null };
  ...
  const achievedResult =
    columns.achieved !== undefined ? toNumber(cells[columns.achieved]) : { value: null };
  const acknowledgmentResult =
    columns.acknowledgment !== undefined
      ? toBoolean(cells[columns.acknowledgment])
      : { value: null };
  const feedbackResult =
    columns.feedback !== undefined ? toStringOrNull(cells[columns.feedback]) : { value: null };
  ...
  // stays null.
  const achieved =
    achievedResult.value === null && achievedResult.issue === undefined ? 0 : achievedResult.value;
